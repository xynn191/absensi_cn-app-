                  ...
                  <div className="flex items-center gap-3 rounded-[22px] border border-slate-200/75 bg-white/76 px-4 py-4 shadow-[0_10px_24px_rgba(15,23,42,0.04)]">
                    ...
                    <div>
                      <p className="text-sm font-semibold text-slate-800">
                        {classFilter === "Semua"
                          ? "Semua kelas"
                          : classes.find((item) => item.class_id === classFilter)?.class_name ?? "Kelas dipilih"}
                      </p>
                      <p className="text-xs leading-5 text-slate-500">
                        {selectedDate ? formatFriendlyDate(dateValue) : "Tanggal monitoring belum dipilih"}
                      </p>
                    </div>
                  </div>
                </div>
  ...
  return (
    <Popover>
      ...
      <PopoverContent sideOffset={10} className="w-auto rounded-[24px] border border-emerald-200/80 bg-[linear-gradient(180deg,#ffffff_0%,#f4fbf7_100%)] p-4 shadow-[0_24px_54px_rgba(15,23,42,0.12)]">
        ...
        <Calendar mode="single" selected={selectedDate} onSelect={onSelectDate} locale={localeID} buttonVariant="ghost" />
      </PopoverContent>
    </Popover>
  );
}
